...
void requireParameter(int status, const char *name, int lineNumber) {
    if (status <= 0) {
        fprintf(stderr, "SDPA parameter file: entry %d (%s) is missing or is not readable\n", lineNumber, name);
...
void Parameter::readFile(FILE *parameterFile) {
    ...
    mpf_set_default_prec(precision);
    requireParameter(fscanf(parameterFile, "%29s %*[^\n]", xPrint), "xPrint", 12);
    requireParameter(fscanf(parameterFile, "%29s %*[^\n]", XPrint), "XPrint", 13);
    requireParameter(fscanf(parameterFile, "%29s %*[^\n]", YPrint), "YPrint", 14);
    requireParameter(fscanf(parameterFile, "%29s %*[^\n]", infPrint), "infPrint", 15);
...
void StepLength::MehrotraCorrector(InputData &inputData, Solutions &currentPt, Phase &phase, Switch &reduction, Newton &newton, AverageComplementarity &mu, RatioInitResCurrentRes &theta, WorkVariables &work, Parameter &param, ComputeTime &com) {
    ...
    // by uninitialised stack reads and an unwritten value here would be one.
    mpf_class incPrimalObj = 0.0;
    mpf_class incDualObj = 0.0;
    ...
    if (reduction.switchType == Switch::ON && (phase.value == SolveInfo::noINFO || phase.value == SolveInfo::pFEAS || phase.value == SolveInfo::dFEAS)) {
        ...
        mpf_class thetaMax = max((1.0 - primal) * theta.primal, (1.0 - dual) * theta.dual);
        mpf_class muNew = mu.current + (primal * uMatzMat + dual * xMatvMat + primal * dual * uMatvMat) / nDim;
        mpf_class alphaMax;
    ...
    if (phase.value == SolveInfo::pdFEAS) {
        ...
        mpf_class objValDual, objValPrimal, maxRatio;
...
//-------------------------------------------------
DirectionParameter::DirectionParameter(mpf_class betaStar) { initialize(betaStar); }

DirectionParameter::~DirectionParameter() {
    // Nothing needs.
}

void DirectionParameter::initialize(mpf_class betaStar) { value = betaStar; }
...
void DirectionParameter::MehrotraCorrector(Phase &phase, StepLength &alpha, Solutions &currentPt, Newton &newton, AverageComplementarity &mu, Parameter &param) {
    ...
    mpf_class muTarget = mu.current + (alpha.primal * uMatzMat + alpha.dual * xMatvMat + alpha.primal * alpha.dual * uMatvMat) / nDim;
...
AverageComplementarity::AverageComplementarity(mpf_class lambdaStar) { initialize(lambdaStar); }
...
SolveInfo::SolveInfo(InputData &inputData, Solutions &currentPt, mpf_class mu0, mpf_class omegaStar) { initialize(inputData, currentPt, mu0, omegaStar); }

SolveInfo::~SolveInfo() {
    // Nothing needs.
}

void SolveInfo::initialize(InputData &inputData, Solutions &currentPt, mpf_class mu0, mpf_class omegaStar) {
    int nDim = currentPt.nDim;
    Vector &b = inputData.b;
    SparseLinearSpace &C = inputData.C;
