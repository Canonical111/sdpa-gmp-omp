...
/* MODIFIED from upstream (GPLv2 2a notice), 2026-08-14: the canonical sparse-matrix stream
   is lifted out of sdpa_linear.cpp's anonymous namespace so the bMat ASSEMBLY can emit the
   same bytes the FACTOR does, from one implementation.

   Why the assembly needs its own stream at all: finite-precision Cholesky is not injective,
   so two different bMat inputs can round to the same completed factor. "The factor stream is
   identical" is strong downstream evidence and is NOT proof that the assembly produced the
   same matrix. Threading the assembly therefore has to be proved against the matrix it
   builds, before that matrix is factored. See review/MAKE-BMAT-THREADING-PLAN.md. */
struct CanonicalStream {
    uint64_t fnv;     // FNV-1a over the framed stream -- a fingerprint, not a proof
    ...
    bool io_error;    // a write failed: what is on disk is NOT what `bytes` claims
};

// Canonical, length-framed serialisation of a row-grouped upper-triangular sparse matrix:
// type, dimensions, all three sparse counts, row extents, and per element its index, row,
// column, precision, sign, exponent and exact base-16 mantissa. `tag` distinguishes streams
// from different producers so an assembly stream can never compare equal to a factor stream.
CanonicalStream canonicalSparseStream(SparseMatrix &aMat, int *diagonalIndex, int nDIM,
                                      const char *tag, FILE *dump);

// Same framing for a solved vector: dimension, then per element its index, precision, sign,
// exponent and exact base-16 mantissa. The oracle for the threaded forward substitution --
// the printed solution carries too few digits to see a last-bit change, which is precisely
// the change a reordered sum would make.
