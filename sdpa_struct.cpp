...
// INT_MAX, which bounds the element count of every dense allocation below.
#include <climits>
...
BlockVector::BlockVector(int nBlock, int *blockStruct, mpf_class value) { initialize(nBlock, blockStruct, value); }

BlockVector::~BlockVector() { terminate(); }

void BlockVector::initialize(int nBlock, int *blockStruct, mpf_class value) {
...
void SparseMatrix::changeToDense(bool forceChange) {
    ...
    /* MODIFIED from upstream (GPLv2 2a notice), 2026-08-12: the same int-overflow fix as
       DenseMatrix::initialize below, plus the density test itself -- upstream computed
       `(nRow * nCol) * 0.20` in `int` before widening, so the threshold this decision
       rests on was also undefined past nRow*nCol > INT_MAX. Both are unreachable for any
       matrix upstream could allocate. */
    if (forceChange == false && NonZeroCount < ((double)nRow * (double)nCol) * 0.20) {
        // if the number of elements are less than 20 percent,
        // we don't change to Dense.
        return;
    }
    if ((double)nRow * (double)nCol > (double)INT_MAX) {
        rError("SparseMatrix::changeToDense: nRow*nCol overflows int (nRow = "
               << nRow << ", nCol = " << nCol << ")");
    }
    mpf_class MZERO = 0.0;
    // rMessage("change");
...
void DenseMatrix::initialize(int nRow, int nCol, DenseMatrix::Type type) {
    ...
    switch (type) {
    case DENSE:
        /* MODIFIED from upstream (GPLv2 2a notice), 2026-08-12: the element count is
           computed in `int`, so nRow*nCol silently overflows for nRow == nCol >= 46341 and
           `new mpf_class[length]` is then reached with a negative length -- undefined
           behaviour rather than an allocation failure, so the `de_ele == NULL` test below
           can never catch it. Chordal::ordering_bMat now refuses to select a dense bMat
           past this limit, but that is not the only caller, so the check is repeated at the
           allocation itself. Unreachable for any matrix upstream could allocate. */
        if ((double)nRow * (double)nCol > (double)INT_MAX) {
            rError("DenseMatrix::initialize: nRow*nCol overflows int (nRow = "
                   << nRow << ", nCol = " << nCol << ")");
        }
        length = nRow * nCol;
...
void SparseLinearSpace::setElement_SOCP(int block, int i, int j, mpf_class ele) { rError("DenseLinearSpace:: current version does not support SOCP"); }
...
void DenseLinearSpace::setElement_SOCP(int block, int i, int j, mpf_class ele) { rError("DenseLinearSpace:: current version does not support SOCP"); }
